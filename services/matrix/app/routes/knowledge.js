...
router.post('/create', function(req, res, next) {
    ...
    ServiceAuth(req.headers['authorization'], function (err, status, scope) {
        if (err) {
...
router.delete('/', function(req, res, next) {
    ...
    ServiceAuth(req.headers['authorization'], function (err, status, scope) {
        ...
        knowledge.delete(function(err, result) {
            ...
            log.info(`SUCCESS - ${req.originalUrl} - ${req.method} - ${req.ip}`);
            return res.status(200).send(T.Data('Deleted ' + result.n + ' items', scope));
        });
    });
});
...
function ServiceAuth(header_auth, callback) {
    if (validator.Validity(header_auth) != null) {
        return passport.CheckServiceAuth(header_auth, callback);
    } else {
        return callback(new Error('Header "authorization" is undefined'), 401, null);  
    }
}
