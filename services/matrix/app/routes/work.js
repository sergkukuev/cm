...
router.post('/create', function(req, res, next) {
    passport.checkServiceAuthorization(req, res, function (scope) {
        ...
        if (tasks.status == "Error")
            return res.status(400).send(format.T(400, tasks.desc));
...
router.put('/:id_work/tasks/:id_task', function(req, res, next) {
    passport.checkServiceAuthorization(req, res, function (scope) {
        ...
        if (!validator.checkId(id_work))
            return res.status(400).send(format.T(400, desc.InvalidId(id_work)));
        if (!validator.checkId(id_task))
            return res.status(400).send(format.T(400, desc.InvalidId(id_task)));

        work.getById(id_work, function(err, result1) {
            if (err)
                res.status(400).send(format.T(400, err));
            else if (!result1)
                res.status(404).send(format.T(404, desc.NotFound(id)));
            else {
                let index = -1;
                for (let i = 0; (i < result1.tasks.length) && index == -1; i++)
                    if (result1.tasks[i].id == id_task)
                        index = i;  
                if (index == -1)
                    return res.status(404).send(format.T(404, "Задачи с таким идентификатором не существует в данной работе"));
                if (!validator.checkUndefined(req.body.name))
