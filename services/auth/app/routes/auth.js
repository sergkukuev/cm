...
router.put('/code', function(req, res, next) {
    ...
    passport.ServiceAuth(req.headers, function(err, scope) {
        if (err) {
            return next(err);
        }
        const data = {
        ...
        if (!data.login || !data.password) {
            return next(TError(null, true, 'Invalid parameter "login" or "password"', 400, scope));
        } else if (!data.response_type) {
            return next(TError(null, true, 'Invalid parameter "grant_type"', 400, scope));
        }
...
function codeAuthorization(req, res, next, service_scope) {
    ...
    if (!code) {
        return next(TError(null, true, 'Invalid parameter "code"', 400, service_scope));
    }
    return passport.SetUTokenByCode(code, function(err, status, user_scope) {
        if (err) {
            return next(TError(err, true, err.status || status, service_scope));
        }
        log.info(`SUCCESS - ${req.originalUrl} - ${req.method} - ${req.ip}`);
...
function passAuthorization(req, res, next, service_scope) {
    ...
    if (!data.login || !data.password)
        return next(TError(null, true, 'Invalid parameters "login" or "password"', 400, service_scope));
...
function rTokenAuthorization(req, res, next, service_scope) {
    ...
    if (!token) {
        return next(TError(null, true, 'Invalid parameter "refresh_token"', 400, service_scope));
    }
